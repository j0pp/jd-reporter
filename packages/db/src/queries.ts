...
const REVIEWABLE: FindingStatus[] = ['detected', 'confirmed', 'needs_review'];
...
export async function ensureBoard(db: Db, input: EnsureBoardInput) {
  ...
  const [company] = await db
    ...
    .values({
      ...
      sectorConfidence: input.sectorConfidence ?? null,
      naics2: input.naics2 ?? null,
...
export async function activeBoardsForCrawl(db: Db, vendor: string, limit?: number) {
  const rows = await db
    .select({ board: boards, hasOpenFinding: sql<number>`exists(select 1 from ${findings} f where f.company_id = ${boards.companyId} and f.status in ('detected','confirmed','needs_review','published'))` })
    .from(boards)
    .where(and(eq(boards.atsVendor, vendor as never), eq(boards.status, 'active')))
    .orderBy(desc(sql`exists(select 1 from ${findings} f where f.company_id = ${boards.companyId} and f.status in ('detected','confirmed','needs_review','published'))`), asc(sql`coalesce(${boards.lastCrawledAt}, '')`));
...
export function reviewReasonsFor(p: { jurisdiction: JurisdictionJson; range: RangeJson; isOffsite: boolean; employmentType: string | null; title: string }): string[] {
  const reasons: string[] = [];
  if (p.jurisdiction.nyc === 'possible' || p.jurisdiction.nys === 'possible') reasons.push('jurisdiction_possible');
  if (p.jurisdiction.multiCity) reasons.push('multi_city');
  if (p.jurisdiction.locClass === 'ny_bare') reasons.push('bare_new_york');
...
// detected on a non-disclosed covered posting, withdrawn when a range shows up before anyone looked,
// fixed when it shows up after publication, stale when a published posting disappears.
export async function reconcileFindings(db: Db, postingIds: number[], ctx: { actor: string }): Promise<ReconcileCounts> {
  ...
  for (let i = 0; i < postingIds.length; i += 50) {
    ...
    for (const p of rows) {
      ...
      const wantType = p.removedAt || p.isEvergreen ? null : coveredFor(p.jurisdiction) ? findingTypeFor(p.range.method) : null;
      ...
      for (const f of fs) {
        ...
        if (p.removedAt && f.status === 'published') {
        ...
        } else if (p.removedAt && REVIEWABLE.includes(f.status)) {
          if (await transitionFinding(db, { id: f.id, from: REVIEWABLE, to: 'withdrawn', actor: ctx.actor, reason: 'posting removed before publication' })) counts.withdrawn += 1;
          ...
          if (await transitionFinding(db, { id: f.id, from: REVIEWABLE, to: 'withdrawn', actor: ctx.actor, reason: `posting now reads as ${p.range.method}` })) counts.withdrawn += 1;
        }
      }
      ...
      if (!same) {
        await db.insert(findings).values({
          ...
          type: wantType,
          status: 'detected',
          jurisdictionCodes: codesFor(p.jurisdiction),
        ...
        // the posting regressed or came back: start the clock again, never reuse the old evidence
        if (
...
function coveredFor(j: JurisdictionJson): boolean {
  return j.nyc === 'covered' || j.nyc === 'possible' || j.nys === 'covered' || j.nys === 'possible';
}

function codesFor(j: JurisdictionJson): string[] {
  const out: string[] = [];
  if (j.nyc === 'covered' || j.nyc === 'possible') out.push('nyc');
  if (j.nys === 'covered' || j.nys === 'possible') out.push('nys');
  return out;
}

// detected -> confirmed when a later successful crawl of the same board (>= 20 h) still agrees
export async function confirmDetectedFindings(db: Db, minAgeHours = 20): Promise<number> {
  const cutoff = new Date(Date.now() - minAgeHours * 3600_000).toISOString();
  const rows = await db
    .select({ id: findings.id, detectedAt: findings.detectedAt, lastOkAt: boards.lastOkAt })
    .from(findings)
    .innerJoin(postings, eq(postings.id, findings.postingId))
    .innerJoin(boards, eq(boards.id, postings.boardId))
    .where(and(eq(findings.status, 'detected'), sql`${findings.detectedAt} <= ${cutoff}`, isNull(postings.removedAt)));
  let n = 0;
  for (const r of rows) {
    // the board must have been read successfully at least 20 h after detection for the second look to count
    if (!r.lastOkAt || new Date(r.lastOkAt).getTime() < new Date(r.detectedAt).getTime() + minAgeHours * 3600_000) continue;
    if (await transitionFinding(db, { id: r.id, from: 'detected', to: 'confirmed', actor: 'system', reason: 'second crawl agreed' })) n += 1;
  }
  return n;
}

export async function queueConfirmedForReview(db: Db): Promise<number> {
  const rows = await db.select({ id: findings.id }).from(findings).where(eq(findings.status, 'confirmed'));
  let n = 0;
  for (const r of rows) {
    if (await transitionFinding(db, { id: r.id, from: 'confirmed', to: 'needs_review', actor: 'system' })) n += 1;
  }
  return n;
}

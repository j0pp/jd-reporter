...
import { classifyCoverage } from './coverage/index.ts';
import { sha256Hex, stableStringify } from './hash.ts';
import type { AtsVendor, CoverageResult, RangeResult, RawPosting } from './types.ts';
...
export function classifyPosting(vendor: AtsVendor, p: RawPosting): Classified {
  const coverage = classifyCoverage({ locations: p.locations, isRemote: p.isRemote, text: p.descriptionText });
  ...
  return { coverage, range, isOffsite: offsite, classifierVersion: CLASSIFIER_VERSION };
}

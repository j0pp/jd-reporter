import type { Coverage, JurisdictionResult, LocClass } from '../types.ts';
...
// three answers per jurisdiction, never two (FINDINGS #2): covered, possible (human decides), no, or
// not_assessed for remote postings until company hq is known
export function classifyJurisdiction(input: JurisdictionInput): JurisdictionResult {
  ...
  if (input.text && hasNy && TEXT_EXCLUDES_NY_RE.test(input.text)) {
    ...
    if (nyc === 'covered') nyc = 'possible';
    if (nys === 'covered') nys = 'possible';
    confidence = Math.min(confidence, 0.5);
  }

  return { nyc, nys, locClass: best, multiCity, remoteUs, reasons, confidence };
}

export function isNySignal(j: JurisdictionResult): boolean {
  return j.locClass === 'nyc_strict' || j.locClass === 'ny_bare' || j.locClass === 'ny_state';
}
